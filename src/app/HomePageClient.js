...
export default function HomePageClient() {
  ...
  const mode = typeof searchParams?.get("mode") === "string" ? searchParams.get("mode") : "vendors";
  const search = searchParams?.get("search") ?? null;
  const category = searchParams?.get("category") ?? null;
  const minPrice = searchParams?.get("minPrice") ?? null;
  const maxPrice = searchParams?.get("maxPrice") ?? null;
  const eventType = searchParams?.get("type") ?? null;
  const eventDate = searchParams?.get("date") ?? null;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    if (mode === "events") {
      const data = await fetchEvents({ search, type: eventType, date: eventDate });
      setEvents(data);
      setVendors([]);
      setAllCategories([]);
    } else {
      const [filtered, all] = await Promise.all([
        listVendors({ search, category, minPrice, maxPrice }),
        listVendors(),
      ]);
      setVendors(filtered);
      setAllCategories(
    ...
    setLoading(false);
  }, [mode, search, category, minPrice, maxPrice, eventType, eventDate]);
    ...
    <div className="w-full">
        ...
        <section className="bg-white border-b border-[var(--gray-100)]">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-5">
            <div className="scroll-horizontal scrollbar-hide gap-4 -mx-4 px-4">
              ...
              />
              {VENDOR_CATEGORIES.filter((c) => allCategories.includes(c)).map((c) => {
                ...
                return (
                  <QuickActionCard
                    ...
                    label={display.label}
                    color={display.color}
                    badge={c === "Food & Beverage" ? "Popular" : undefined}
                  />
                );
              })}
            </div>
      ...
      {mode === "events" && (
        <section className="bg-white border-b border-[var(--gray-100)]">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-5">
            <div className="scroll-horizontal scrollbar-hide gap-4 -mx-4 px-4">
              <QuickActionCard href="/?mode=vendors" icon="🏪" label="Vendors" color="var(--violet-50)" />
              <QuickActionCard href="/?mode=events&type=party" icon="🎉" label="Parties" color="var(--magenta-100)" badge="Hot" />
              <QuickActionCard href="/?mode=events&type=wedding" icon="💒" label="Weddings" color="var(--coral-100)" />
              <QuickActionCard href="/?mode=events&type=concert" icon="🎸" label="Concerts" color="var(--amber-100)" />
              <QuickActionCard href="/?mode=events&type=festival" icon="🎪" label="Festivals" color="var(--info-50)" />
              <QuickActionCard href="/?mode=events&type=corporate" icon="💼" label="Corporate" color="var(--violet-50)" />
              <QuickActionCard href="/?mode=events&type=wellness" icon="🧘" label="Wellness" color="var(--mint-100)" />
            </div>
          </div>
        </section>
      )}
        ...
        <section className="bg-[var(--gray-50)] border-b border-[var(--gray-100)]">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {search && <FilterBadge label={`"${search}"`} href="/" />}
              {category && (
              ...
              <Link href="/" className="text-sm font-medium text-[var(--violet-600)] hover:text-[var(--violet-700)] ml-2">
                Clear all
              </Link>
            </div>
          </div>
      ...
      {mode === "events" && (search || eventType || eventDate) && (
        <section className="bg-[var(--gray-50)] border-b border-[var(--gray-100)]">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-3">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm text-[var(--gray-500)]">Active filters:</span>
              {search && <FilterBadge label={`"${search}"`} href="/?mode=events" />}
              {eventType && <FilterBadge label={eventType} href="/?mode=events" />}
              {eventDate && <FilterBadge label={eventDate} href="/?mode=events" />}
              <Link href="/?mode=events" className="text-sm font-medium text-[var(--violet-600)] hover:text-[var(--violet-700)] ml-2">
                Clear all
              </Link>
            </div>
          </div>
      ...
      {mode === "vendors" && (
        <div className="py-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-[var(--gray-900)]">
                  {search ? `Results for "${search}"` : category ? `${category} Vendors` : "All Vendors"}
                </h2>
                <p className="text-sm text-[var(--gray-500)]">
                  {loading ? "Loading…" : `${vendors.length} vendor${vendors.length !== 1 ? "s" : ""} ${search ? "found" : "available"}`}
                </p>
              </div>
              <Link href="/vendors" className="text-sm font-medium text-[var(--violet-600)] hover:text-[var(--violet-700)]">
                View all →
              </Link>
            </div>
          </div>
          <div className="space-y-10">
            {loading ? (
              <div className="mx-auto max-w-6xl px-4 sm:px-6 text-center py-12 text-[var(--gray-500)]">Loading vendors…</div>
        ...
        <div className="py-6">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 mb-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-[var(--gray-900)]">
                  {search ? `Events matching "${search}"` : eventType ? `${eventType.charAt(0).toUpperCase() + eventType.slice(1)} Events` : "Upcoming Events"}
                </h2>
                <p className="text-sm text-[var(--gray-500)]">
                  {loading ? "Loading…" : `${events.length} event${events.length !== 1 ? "s" : ""} ${search ? "found" : "near you"}`}
                </p>
              </div>
...
function QuickActionCard({ href, icon, label, color, badge }) {
  return (
    <Link href={href} className="flex flex-col items-center gap-2 min-w-[72px] pt-2">
      <div
  ...
  return (
    <section className="animate-slide-up mx-auto max-w-6xl px-4 sm:px-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-semibold text-[var(--gray-900)]">{category}</h3>
        <Link
...
function VendorCard({ vendor, index }) {
  ...
  return (
    <Link href={getStorefrontPath(vendor)} className="group flex-shrink-0 w-[200px] sm:w-[220px]">
      ...
      <div>
        <h4 className="font-semibold text-sm text-[var(--gray-900)] line-clamp-1 group-hover:text-[var(--violet-600)] transition-colors">{vendor.name}</h4>
        <p className="text-xs text-[var(--gray-500)] line-clamp-1 mt-0.5">{vendor.description}</p>
...
function EventCard({ event, index }) {
  ...
  return (
    <Link href={`/events?eventId=${event.id}`} className="group">
      <div className="bg-white rounded-[var(--radius-2xl)] shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-card-hover)] transition-all overflow-hidden">
        <div className="relative aspect-[16/9]">
          ...
          {eventDate && (
            <div className="absolute top-3 left-3 bg-white rounded-[var(--radius-lg)] p-2 shadow-lg text-center min-w-[50px]">
              ...
              <div className="text-lg font-bold text-[var(--gray-900)]">{eventDate.getDate()}</div>
            </div>
          )}
          {event.status && (
        ...
        <div className="p-4">
          <h3 className="font-semibold text-[var(--gray-900)] line-clamp-1 group-hover:text-[var(--violet-600)] transition-colors mb-1">{event.name}</h3>
          {eventDate && (
          ...
          {event.attendee_count > 0 && (
            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[var(--gray-100)]">
              ...
              <span className="text-xs text-[var(--gray-500)]">{event.attendee_count}+ going</span>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="text-center py-16">
      ...
      <p className="text-[var(--gray-500)] mb-6 max-w-sm mx-auto">
        {isEvents ? "Try adjusting your filters or check back later for new events." : "Try adjusting your filters or browse all vendors to discover something new."}
      </p>
      <Link href={isEvents ? "/?mode=events" : "/"} className="btn btn-gradient inline-flex">
        Clear Filters
      </Link>
    </div>
  );
