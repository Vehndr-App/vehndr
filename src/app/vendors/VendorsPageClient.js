...
function VendorsListContent({ searchQuery, category, minPrice, maxPrice }) {
  ...
  const load = useCallback(async () => {
    ...
    const [filtered, all] = await Promise.all([
      listVendors({ search: searchQuery, category, minPrice, maxPrice }),
      listVendors(),
    ]);
    setVendors(filtered);
    setAllCategories(
  ...
  const displayedVendorCount = new Set(
    displayedVendors.map((v) => v?.id).filter(Boolean)
  ).size;
    ...
    <div className="w-full">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6">
          <div className="mb-4">
            ...
            <p className="text-[var(--gray-500)] text-sm mt-1">
              {loading ? "Loading…" : `${displayedVendorCount} vendor${displayedVendorCount !== 1 ? "s" : ""} available`}
            </p>
          </div>
        ...
        <div className="bg-white border-b border-[var(--gray-100)]">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Link href="/vendors" className="text-sm font-medium text-[var(--violet-600)] hover:text-[var(--violet-700)] ml-2">
                Clear all
              </Link>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6">
        <div className="space-y-10">
          {loading ? (
            <div className="text-center py-12 text-[var(--gray-500)]">Loading vendors…</div>
...
function VendorCard({ vendor, index }) {
  ...
  return (
    <Link href={getStorefrontPath(vendor)} className="group">
      ...
      <div>
        <h3 className="font-semibold text-sm text-[var(--gray-900)] line-clamp-1 group-hover:text-[var(--violet-600)] transition-colors">{vendor.name}</h3>
        <p className="text-xs text-[var(--gray-500)] line-clamp-2 mt-0.5 min-h-[32px]">{vendor.description}</p>
...
function EmptyState() {
  return (
    <div className="text-center py-16">
      ...
      <Link href="/vendors" className="btn btn-gradient inline-flex">
        Clear Filters
      </Link>
    </div>
  );
